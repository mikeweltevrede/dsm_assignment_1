...
Penalty <- rep(0, ncol(PCA_sul$rotation)-1)
...
for (k in 1:(ncol(PCA_sul$rotation)-1)) {
  ...
  for (i in 1:nrow(PCA_sul$x)) {
    for (j in 1:nrow(PCA_sul$rotation)) {
      epsilon2[i,j] <- (data_sul$scaled_data[i,j] - PCA_sul$rotation[j,1:k]%*%t(PCA_sul$x)[1:k,i])^2
    }
  }
...
for (k in 1:(ncol(PCA_sul$rotation)-1)) {
  ...
  for (i in 1:nrow(PCA_sul$x)) {
    for (j in 1:nrow(PCA_sul$rotation)) {
      epsilon2[i,j] <- (data_sul$selected_data_02[i,j] - PCA_sul$rotation[j,1:k]%*%t(PCA_sul$x)[1:k,i])^2
    }
  }
  SSR[k] <- sum(epsilon2)/(nrow(PCA_sul$x)*nrow(PCA_sul$rotation))
  Penalty[k] <- k*log(Cnp)/Cnp
...
title(xlab = 'Number of PC', ylab = 'BIC' )
...
data_PC1 <- melt(data_PC1, id.vars='years')
